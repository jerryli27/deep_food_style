...
class Vgg19:
    ...
    def __init__(self, vgg19_npy_path=None, trainable=True):
        if vgg19_npy_path is not None:
            self.data_dict = {}
            data = scipy.io.loadmat(vgg19_npy_path)
            ...
            for i, name in enumerate(layers):
                ...
                if kind == 'conv':
                    ...
                    bias = bias.reshape(-1)
                    self.data_dict[name]=[kernels, bias]
                else:
                    pass
        else:
            self.data_dict = None
    ...
    def build(self, rgb, output_classes, trainable_layer='fc6', train_mode=None):
        ...
        rgb_scaled = rgb * 255.0 - np.array(VGG_MEAN)

        # assert rgb_scaled.get_shape().as_list()[1:] == [224, 224, 3]
        height = rgb_scaled.get_shape().as_list()[1]
        width = rgb_scaled.get_shape().as_list()[2]
        if trainable_layer == "conv1_1":
            self.trainable = True
        self.conv1_1 = self.conv_layer(rgb_scaled, 3, 64, "conv1_1")
    ...
    def save_npy(self, sess, npy_path="./vgg19-save.npy"):
        ...
        #
        #
        #     if not data_dict.has_key(layer_name):
        #         data_dict[layer_name] = [None,None]
        #
        #     if "_biases" in name:
        #         data_dict[layer_name][1] = var_out.reshape(-1)
        #     else:
        #         data_dict[layer_name][0] = np.transpose(var_out, (1, 0, 2, 3))
        #
